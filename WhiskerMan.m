            ...
            for w = 1:handles.Nwhiskers
                ...
                for attempt = 1:nattempts
                    ...
                    [z,Emin,exitflag,output] = fminunc(@(z) Bimfun_wm4(z, r3, np0, np1, im.h, im.v, handles.dt, ...
                        ...
                        handles.options);
                    clear exitflag output
                    fprintf('\nFinal estimate from fminunc: %g at %g %g %g %g %g %g %g\n', Emin, z)
                    
                    % Initialise variables needed for e04dg
                    ...
                    user.imv = im.v;
                    user.dt = handles.dt;
                    user.sigma1 = handles.trpmtrs.sigma_prior;
                    user.sigma2 = handles.trpmtrs.sigma2_prior;
                    user.calib = handles.calib;
                    [iter, objf, objgrd, x, user, lwsav, iwsav, rwsav, ifail] = e04dg(@objf_wrapper, z0, lwsav, iwsav, rwsav, 'user', user);
                    fprintf('\nFinal estimate of solution using NAG: %g at %g %g %g %g %g %g %g\n', objf, x)
